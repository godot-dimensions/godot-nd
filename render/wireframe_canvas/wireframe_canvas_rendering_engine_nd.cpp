...
Color WireframeCanvasRenderingEngineND::_get_material_edge_color(const Ref<MaterialND> &p_material, const Ref<SingleSurfaceMeshND> &p_mesh, int p_edge_index) {
		...
		for (int surface_mesh_index = 0; surface_mesh_index < surface_meshes.size(); surface_mesh_index++) {
			...
			for (int edge_index = 0; edge_index < edge_indices.size() / 2; edge_index++) {
				...
				if (direct_project) {
					...
					edge_color = _get_material_edge_color(material_nd, surface_mesh_nd, edge_index);
				} else {
			...
			const Ref<WireMaterialND> wire_material_nd = material_nd;
			if (wire_material_nd.is_valid()) {
				edge_thicknesses_to_draw.push_back(wire_material_nd->get_line_thickness() > 0.0 ? wire_material_nd->get_line_thickness() : -1.0);
			} else {
