...
void ArrayPolyMeshND::_delete_poly_cell_element_internal(const int32_t p_poly_cell_index, const int32_t p_index) {
	...
	for (KeyValue<Vector2i, Vector<Vector<VectorN>>> &normals_iterator : _all_poly_cell_normals) {
		const Vector2i key = normals_iterator.key;
		if (key.x == geom_dim) {
			Vector<Vector<VectorN>> &normals_for_dim = normals_iterator.value;
			if (key.y == key.x) {
				// Non-decomposed case. Only one array, with one normal per cell.
				if (!normals_for_dim.is_empty() && p_index < normals_for_dim[0].size()) {
					normals_for_dim.ptrw()[0].remove_at(p_index);
				}
			} else if (p_index < normals_for_dim.size()) {
	...
	for (KeyValue<Vector2i, Vector<Vector<VectorM>>> &texture_map_iterator : _all_poly_cell_texture_maps) {
		const Vector2i key = texture_map_iterator.key;
		if (key.x == geom_dim) {
			Vector<Vector<VectorM>> &texture_map_for_dim = texture_map_iterator.value;
			if (key.y == key.x) {
				// Non-decomposed case. Only one array, with one texture coordinate per cell.
				if (!texture_map_for_dim.is_empty() && p_index < texture_map_for_dim[0].size()) {
					texture_map_for_dim.ptrw()[0].remove_at(p_index);
				}
			} else if (p_index < texture_map_for_dim.size()) {
				// Decomposed case, such as vertex texture coordinates for each cell.
...
void ArrayPolyMeshND::merge_with(const Ref<PolyMeshND> &p_other, const Ref<TransformND> &p_transform) {
		...
		if (missing_entries) {
			if (boundary_dim_index >= 0 && poly_cell_indices_dims > boundary_dim_index && key == per_cell_key) {
				...
				if (key.y == key.x) {
					...
					Vector<VectorN> flat_merged_normals;
	...
	if (boundary_dim_index >= 0 && _poly_cell_indices.size() > boundary_dim_index && _all_poly_cell_normals.has(per_cell_key)) {
		...
		if (merged_boundary_normals.is_empty()) {
			merged_boundary_normals.append(Vector<VectorN>());
		}
	...
	if (boundary_dim_index >= 0 && _poly_cell_indices.size() > boundary_dim_index && _all_poly_cell_normals.has(cell_to_vert_key)) {
