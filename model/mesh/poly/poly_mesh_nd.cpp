...
void PolyMeshND::_decompose_boundary_cells_into_simplexes() {
	...
	Vector<PackedInt32Array> boundary_cell_vertex_indices = _get_boundary_cell_vertex_indices_cached(true);
...
const Vector<PackedInt32Array> &PolyMeshND::_get_boundary_cell_vertex_indices_cached(const bool p_start_with_canonical_span) {
	...
	if (cache.is_empty()) {
		...
		const Vector<Vector<PackedInt32Array>> poly_cell_indices = get_poly_cell_indices();
		if (boundary_dim_index < 0 || poly_cell_indices.size() <= boundary_dim_index) {
			return cache; // No boundary cells, so there is nothing to cache.
		}
		cache = _get_vertex_indices_of_boundary_cells(poly_cell_indices, get_edge_indices(), boundary_dim_index, p_start_with_canonical_span);
	}
	return cache;
}
...
HashMap<Vector2i, Vector<PackedInt32Array>> PolyMeshND::get_all_poly_cell_normal_indices() {
	...
	if (!vert_normal_indices.is_empty()) {
		ret.insert(_get_cell_to_vert_key(), vert_normal_indices);
	}
	// Note: Callers may wish to also append `get_poly_cell_boundary_normals` as (N-1, N-1), but this requires modifying the values array as well.
	return ret;
}

HashMap<Vector2i, Vector<PackedInt32Array>> PolyMeshND::get_all_poly_cell_texture_map_indices() {
	HashMap<Vector2i, Vector<PackedInt32Array>> ret;
	...
	if (!vert_texture_map_indices.is_empty()) {
		ret.insert(_get_cell_to_vert_key(), vert_texture_map_indices);
	}
	return ret;
}

PolyMeshND::PolyDataDictionary PolyMeshND::get_all_poly_cell_normal_indices_bind() {
...
PolyMeshND::PolyDataDictionary PolyMeshND::get_all_poly_cell_texture_map_indices_bind() {
	PolyDataDictionary result;
...
Vector<PackedInt32Array> PolyMeshND::get_all_poly_cell_vertex_indices(const int p_cell_dimension, const bool p_start_with_canonical_span) {
	...
	if (p_cell_dimension - 2 == _get_boundary_poly_dim_index()) {
		// The (N-1)-dimensional cells are the boundary cells, whose traversal is cached.
		return _get_boundary_cell_vertex_indices_cached(p_start_with_canonical_span);
	}
...
void PolyMeshND::_poly_mesh_clear_simplex_cache_internal(const bool p_normals_only) {
	...
	if (_simplex_cell_texture_map_indices_cache.is_empty()) {
		...
		if (has_cell_to_vert) { // Boundary cells decomposed into vertices.
			...
			if (wrapper.size() != 1) {
				...
				ERR_FAIL_V_MSG(PackedInt32Array(), "PolyMeshND: The per-cell texture map binding must contain exactly one array of indices, but it contains " + itos(wrapper.size()) + ".");
			}
			const PackedInt32Array &poly_cell_texture_map_indices = wrapper[0];
			for (int64_t simplex_index = 0; simplex_index < simplex_count; simplex_index++) {
				const int32_t source_cell = _simplex_cell_source_poly_cells[simplex_index];
				const int64_t simplex_start = simplex_index * dimension;
				...
				if (source_cell >= poly_cell_texture_map_indices.size()) {
					...
					for (int64_t vert_in_simplex = 0; vert_in_simplex < dimension; vert_in_simplex++) {
...
void PolyMeshND::_bind_methods() {
	ClassDB::bind_method(D_METHOD("get_all_poly_cell_normal_indices"), &PolyMeshND::get_all_poly_cell_normal_indices_bind);
