...
TEST_CASE("[ArrayPolyMeshND] Repeated merges do not accumulate unreferenced pool values") {
	for (int dimension = 3; dimension <= 5; dimension++) {
		...
		for (int merge_index = 1; merge_index <= 3; merge_index++) {
			...
			CAPTURE(merge_index);
			const Vector<VectorN> normal_values = mesh->get_poly_cell_normal_values();
			const Vector<VectorM> texture_values = mesh->get_poly_cell_texture_map_values();
			// Pool growth would show up here as an unexpected size, distinct from a content mismatch.
			REQUIRE(normal_values.size() == 1);
			REQUIRE(texture_values.size() == 1);
			CHECK(VectorND::is_equal_exact(normal_values[0], normal));
			CHECK(VectorND::is_equal_exact(texture_values[0], texture));
